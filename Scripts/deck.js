    ...
    createDeck() {
        ...
        for (let i = 0; i < 5; i++) {
            for (let j = 0; j < Card.colors.length; j++) {
                this.cartes.push(new Organ(Card.colors[j],'./imatges/organo'+Card.colors[j]+'.png'));
            }
        }
       ...
       for (let i = 0; i < 4; i++) {
            for (let j = 0; j < Card.colors.length; j++) {
                this.cartes.push(new Medicina(Card.colors[j],'./imatges/medicina'+Card.colors[j]+'.png'));
            }
        }
       while(cont<4){
